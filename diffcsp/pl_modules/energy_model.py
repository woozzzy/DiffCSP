...
class CSPEnergy(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.decoder = hydra.utils.instantiate(
            self.hparams.decoder, latent_dim=self.hparams.time_dim, pred_scalar=True, smooth=True
        )
    ...
    def sample(self, batch, uncod, diff_ratio=1.0, step_lr=1e-5, aug=1.0):
        ...
        t_T = (
            torch.randn([batch.num_nodes, MAX_ATOMIC_NUM]).to(self.device)
            if update_type
            else F.one_hot(batch.atom_types - 1, num_classes=MAX_ATOMIC_NUM).float()
        )
        ...
        traj = {
            time_start: {"num_atoms": batch.num_atoms, "atom_types": t_T, "frac_coords": x_T % 1.0, "lattices": l_T}
        }
        ...
        for t in tqdm(range(time_start, 0, -1)):
            ...
            if update_type:
                ...
                pred_l, pred_x, pred_t = uncod.decoder(
                ...
                pred_l, pred_x = uncod.decoder(
                    time_emb, t_t_one, x_t_minus_05, l_t_minus_05, batch.num_atoms, batch.batch
                )
                ...
                with torch.enable_grad():
                    with RequiresGradContext(x_t_minus_05, l_t_minus_05, requires_grad=True):
                        ...
                        grad_outputs = [torch.ones_like(pred_e)]
                        grad_x, grad_l = grad(
                            pred_e, [x_t_minus_05, l_t_minus_05], grad_outputs=grad_outputs, allow_unused=True
                        )
    ...
    def type_loss(self, pred_atom_types, target_atom_types, noised_atom_types, batch, times):
        ...
        kldiv = F.kl_div(input=theta_hat, target=theta, reduction="none", log_target=False).sum(dim=-1)
