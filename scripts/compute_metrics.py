        ...
        if np.isnan(self.lengths).any() or np.isnan(self.angles).any() or np.isnan(self.frac_coords).any():
            ...
            try:
                self.structure = Structure(
                    lattice=Lattice.from_parameters(*(self.lengths.tolist() + self.angles.tolist())),
...
class GenEval(object):
    ...
    def get_coverage(self):
        ...
        (cov_metrics_dict, combined_dist_dict) = compute_cov(
            self.crys, self.gt_crys, struc_cutoff=cutoff_dict["struc"], comp_cutoff=cutoff_dict["comp"]
        )
    ...
    if "input_data_batch" in data:
        ...
        if isinstance(batch, dict):
            true_crystal_array_list = get_crystals_list(
                batch["frac_coords"], batch["atom_types"], batch["lengths"], batch["angles"], batch["num_atoms"]
            )
        else:
            true_crystal_array_list = get_crystals_list(
                batch.frac_coords, batch.atom_types, batch.lengths, batch.angles, batch.num_atoms
